...
class Outputs(CoreModel):
    selected_sentences: list[str]= Field(..., description= "List of top k elected sentences")
...
async def main(inputs : Inputs, params : Params) -> Outputs: 
    ...
    query_embedding = model.encode([params.query], batch_size = 100, show_progress_bar = True, convert_to_numpy= True)[0]
    corpus_embeddings = model.encode(inputs.sentences,  batch_size = 100, show_progress_bar = True, convert_to_numpy= True)
    similarity_scores = cosine_similarity([query_embedding], corpus_embeddings)[0]
    sorted_indices = np.argsort(similarity_scores)[::-1]

    return Outputs(selected_sentences=[inputs.sentences[i] for i in sorted_indices[:params.top_k]])
