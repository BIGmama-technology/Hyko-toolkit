...
class Params(CoreModel):
    ...
    replace_mode: ReplaceMode = Field(..., description="Replace mode: replaceAll or replaceFirst")
...
async def main(inputs: Inputs , params: Params)-> Outputs:
    if params.replace_mode == ReplaceMode.replaceAll:
        ...
        replaced_text = inputs.text.replace(params.old_substring, params.new_substring, 1)
    else:
        raise HTTPException(
                    status_code=500,
                    detail=f"Invalid relace mode'{params.replace_mode}'."
                )
