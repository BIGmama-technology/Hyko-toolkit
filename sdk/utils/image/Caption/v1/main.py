...
class Params(CoreModel):
    ...
    caption_color: CaptionColor = Field(..., description="Caption color: white or black")
...
def add_caption(
    img: np.ndarray, caption: str, size: int, position: CaptionPosition, color: CaptionColor
) -> np.ndarray:

    font = cv2.FONT_HERSHEY_SIMPLEX

    if size < 0:
        raise HTTPException(status_code=500, detail="Caption size must be a non-negative integer")
    else: 
        font_scale = size / 100
        font_thickness = 2
    ...
    if color == CaptionColor.BLACK:
        ...
        font_color = (255, 255, 255)  
         ...
         raise HTTPException(status_code=500, detail="Caption color must be 'black' or 'white'")
    ...
    if position == CaptionPosition.TOP:
        ...
        caption_y = img.shape[0] - 10
         ...
         raise HTTPException(status_code=500, detail="Caption position must be 'top' or 'bottom'")
