...
class Params(CoreModel):
    ...
    negative_space_fill: FillColor = Field(..., description="Fill color for shifted area")
...
def get_h_w_c(image):
    ...
    if len(image.shape) == 2:
        ...
        raise HTTPException(
                status_code=500,
                detail=f"Unsupported image shape"
        )
...
def convert_to_BGRA(img: np.ndarray, in_c: int) -> np.ndarray:
    ...
    if in_c not in (1, 3, 4):
        raise HTTPException(status_code=500, detail=f"Number of channels ({in_c}) unexpected")
