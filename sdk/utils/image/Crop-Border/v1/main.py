...
class Params(CoreModel):
    amount: int = Field(..., description="Number of pixels to drop from all four borders")
...
async def main(inputs: Inputs , params: Params)-> Outputs:
    ...
    if params.amount < 0:
        raise HTTPException(
            status_code=500,
            detail="Amount must not be less than 0"
        )
    
    cropped_image = image_cv2[params.amount:height-params.amount, params.amount:width-params.amount]
